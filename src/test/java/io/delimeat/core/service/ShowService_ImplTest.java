...
public class ShowService_ImplTest {
   ...
   private static final SimpleDateFormat SDF = new SimpleDateFormat("yyyy-MM-dd");
	...
	public void createWrongGuideSourceTest() throws Exception {
		ShowDao mockedShowDao = Mockito.mock(ShowDao.class);
		Show show = new Show();
      // this guide source should be ignored because it is does not match the guideDao
	...
	public void createGuideSourceTest() throws Exception {
      ...
      // should be guideId = 2 because that is the correct GuideSource
     	Mockito.when(mockedGuideDao.episodes("2")).thenReturn(Arrays.asList(ep1,ep2,ep3,ep5,ep4,ep6,ep7));
      service.setGuideDao(mockedGuideDao);
     
		Show actualShow = service.create(new Show());
     	Mockito.verify(mockedShowDao).createOrUpdate(Mockito.any(Show.class));
      Mockito.verify(mockedGuideDao).episodes("2");// should be guideId = 2 because that is the correct GuideSource
      Mockito.verify(mockedShowDao, Mockito.times(2)).createOrUpdateEpisode(Mockito.any(Episode.class));
		Assert.assertEquals(show, actualShow); // show returned by showDao should match the show returned by the service
	}
